...
// n: number of colomns(number of variables)
...
int result(float **ab, float *res, int m, int n, int z)
{
	...
	if(z==1)
	{
		...
		//determining whether it is a basic solution or a basic feasible solution
		for(i=0; i<count; i++)
		{
			if(res[i]<0)
				l=1;
...
void solve_linear_eq(float **a, float *b, int *per_arr, float *obj_fn, float *res, float *value, int choice, int m, int n)
{
	...
	//printf("inside linear eq\n");
	//initialising temp_value with +/- INF
	if(choice==0)
	...
	//converting the system of linear equations to row echleon form
	for(i=0; i<m-1; i++)
	{
		if(ab[i][i] == 0)
...
int main()
{
	...
	scanf("%d",&m);
	//number of colomns = number of variables
	printf("enter the number of colomns for matrix A: ");
	scanf("%d",&n);
	
	//calculating the number of basic variables and non-basic variables
	num_nbv = n-m;
	num_bv = m;
	
	//forming the matrix to hold the results
	res = (float *)calloc(m, sizeof(float));
	
	//1 for maximise and 0 for minimize
	printf("1: maximise, 0: minimise\nchoice: ");
	scanf("%d",&choice);
	
	//initialising the value with +/- INF
	if(choice==0)
	...
	//solving the system of linear equation formed after introduction of non-basic variables
	i=0;
	do
	{
		printf("iteration %d\n", ++i);
